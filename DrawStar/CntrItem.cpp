...
// CntrItem.cpp : CDrawStarCntrItem 类的实现
//
...
BOOL CDrawStarCntrItem::OnChangeItemPosition(const CRect& rectPos)
{
	...
	// 就地激活期间，服务器调用 CDrawStarCntrItem::OnChangeItemPosition
	//  以更改就地窗口的位置。
	//  这通常是由于服务器
	//  文档中的数据更改而导致范围改变或是
	//  就地调整大小所引起的。
...
void CDrawStarCntrItem::OnGetItemPosition(CRect& rPosition)
{
	...
	// 就地激活期间，将调用 CDrawStarCntrItem::OnGetItemPosition
	//  以决定该项的位置。  通常，
	//  该矩形会反映与激活所用视图 
	//  相关项的当前位置。  调用 
	//  CDrawStarCntrItem::GetActiveView 可得到视图。
...
void CDrawStarCntrItem::Serialize(CArchive& ar)
{
	...
	// 因为这样会设置从 CDrawStarCntrItem::GetDocument 返回的 m_pDocument 指针，
	//  所以最好先调用
	//  基类 Serialize。
	COleClientItem::Serialize(ar);

	// 现在存储/检索特定于 CDrawStarCntrItem 的数据
	if (ar.IsStoring())
	{
		// TODO: 在此添加存储代码
	}
